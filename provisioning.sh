...
xargs -n 1 -P 3 wget --content-disposition --trust-server-names --no-verbose --header="Authorization: Bearer ${TOKEN_HUGGINGFACE_API}" -P "/app/models/Stable-diffusion" <<< "
...
xargs -n 1 -P 3 wget --content-disposition --trust-server-names --no-verbose --header="Authorization: Bearer ${TOKEN_HUGGINGFACE_API}" -P "/app/models/text_encoder" <<< "
...
# Qwen-Image LORA
xargs -n 1 -P 3 wget --content-disposition --trust-server-names --no-verbose --header="Authorization: Bearer ${TOKEN_HUGGINGFACE_API}" -P "/app/models/Lora" <<< "
